...
async function registerAskModeOff(mock) {
  await mock.commands.ask.handler("off", { hasUI: false, ui: { notify() {} } });
}

test("pi entrypoint registers the shared command policies", async () => {
  const mock = createMockAPI();
  piExtension(mock.pi);
  await registerAskModeOff(mock);
...
test("pi entrypoint preserves tool-result annotation and /ask registration", async () => {
  const mock = createMockAPI();
  piExtension(mock.pi);
  assert.equal(typeof mock.commands.ask?.handler, "function");
